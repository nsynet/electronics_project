...
{   unsigned int length,i;
	...
	if(y0==y1)
	{	
	  	*(__IO uint16_t *) (Bank1_LCD_C)= 0x16;	
	  	*(__IO uint16_t *) (Bank1_LCD_D)= 0x68;	 	 //横屏显示

      	*(__IO uint16_t *) (Bank1_LCD_C)= 2;				
  	  	*(__IO uint16_t *) (Bank1_LCD_D)= 0;		 //设置起始X坐标高字节

  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 3;	
      	*(__IO uint16_t *) (Bank1_LCD_D)= 0;		 //设置起始X坐标低字节

  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 4;				
  	  	*(__IO uint16_t *) (Bank1_LCD_D)= 399>>8;	 	 //设置结束X坐标高字节
	  
      	*(__IO uint16_t *) (Bank1_LCD_C)= 5;	
      	*(__IO uint16_t *) (Bank1_LCD_D)= 399;	 	 //设置结束X坐标低字节

  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 6;				
  	  	*(__IO uint16_t *) (Bank1_LCD_D)= 0;		 //设置起始Y坐标高字节

  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 7;	
  	  	*(__IO uint16_t *) (Bank1_LCD_D)= 0;		 //设置起始Y坐标低字节
    
  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 8;				
      	*(__IO uint16_t *) (Bank1_LCD_D)= 0;	     //设置终止Y坐标高字节

      	*(__IO uint16_t *) (Bank1_LCD_C)= 9;	
      	*(__IO uint16_t *) (Bank1_LCD_D)= 239;	 	 //设置终止Y坐标低字节	  
 
      	*(__IO uint16_t *) (Bank1_LCD_C)= 2;				
  	  	*(__IO uint16_t *) (Bank1_LCD_D)= x0>>8;	 //设置起始X坐标高字节

  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 3;	
      	*(__IO uint16_t *) (Bank1_LCD_D)= (u8)x0;	 //设置起始X坐标低字节	
  
  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 6;				
  	  	*(__IO uint16_t *) (Bank1_LCD_D)= y0>>8;	 //设置起始Y坐标高字节

  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 7;	
  	  	*(__IO uint16_t *) (Bank1_LCD_D)= (u8)y0;	 //设置起始Y坐标低字节
		
	  	*(__IO uint16_t *) (Bank1_LCD_C)= 0x22;	 	 //GRAM写控制
	  
	  length=x1-x0+1;  
  		
 	  for(i=0;i<length;i++)  *(__IO uint16_t *) (Bank1_LCD_D)= color; 	 //开始连续写入    
 	...
 	if(x0==x1)
	{
      	...
      	*(__IO uint16_t *) (Bank1_LCD_D)= (u8)x0;	//设置起始X坐标低字节
  	  	...
  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 4;				
  	  	*(__IO uint16_t *) (Bank1_LCD_D)= x0>>8;	 //设置结束X坐标高字节
	  
      	*(__IO uint16_t *) (Bank1_LCD_C)= 5;	
      	*(__IO uint16_t *) (Bank1_LCD_D)=(u8)x0;	 //设置结束X坐标低字节
  	  	...
  	  	*(__IO uint16_t *) (Bank1_LCD_C)= 8;				
      	*(__IO uint16_t *) (Bank1_LCD_D)= 0x00;	     //设置终止Y坐标高字节
