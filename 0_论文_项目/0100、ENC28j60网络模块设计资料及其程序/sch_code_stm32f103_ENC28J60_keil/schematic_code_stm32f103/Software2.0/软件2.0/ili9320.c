...
u16 DeviceCode;
u16 BackColor = 0x0000;
u16 FrontColor = 0xFFFF;
u8 ttybuf[64];
...
{ 
    ...
    GPIO_InitStructure.GPIO_Pin = nCsPin|RsPin|nWrPin|nRdPin|nRstPin;
	GPIO_Init(GPIOB, &GPIO_InitStructure);  

	GPIO_InitStructure.GPIO_Pin = Lcd_LightPin;
...
///////////////////////////////////////////////////////      
        ...
        ili9320_WriteRegister(0x0061,0x0003); 
...
{
    ili9320_FillRect(0,y * 16,239,y * 16 + 16, BackColor);     
}


void ili9320_putc(u8 c)
{
    static u16 scrollline = 0;
    static u8 lastch = 0;
    if(curx == 0 && scrollline)
    {
        ili9320_WriteRegister(0x006A,scrollline);//垂直GRAM起始位置
        //清空最后一行
        ili9320_FillRect(0,scrollline - 16,239,scrollline, BackColor);   
        scrollline += 16;         
    }
#ifdef __USE_CH_FONT__
    if(c & 0x80)
        lastch ++;

    if(lastch & 0x01)
    {
        if(curx == 232) //如果是双字节汉字,并只剩下一个字符，则换行
        {
            curx = 0;
            cury += 16;            
    ...
    {
        #ifdef __USE_CH_FONT__
        ili9320_PutChar_cf(curx,cury ,c,FrontColor,BackColor);
        #else
        ili9320_PutChar_f(curx,cury ,c,FrontColor,BackColor);
        #endif
        curx += 8;
    }
    if(curx >= 240)
...
void ili9320_PutChar_f(u16 x,u16 y,u8 c,u16 charColor,u16 bkColor)
{
    ...
    ili9320_SetWindows(x,y,x + 7,y + 15);
    ili9320_SetCursor(x,y);
    ...
    for (i=0;i<16;i++)
    {
        ...
        for (j=0;j<8;j++)
        {
            if ( (tmp_char >> 7-j) & 0x01 == 0x01)
            {
                ili9320_WriteData(charColor);                   
            }
            else
            {
                ili9320_WriteData(bkColor);   
            }
            Clr_nWr;Set_nWr;
        }
    }
...
void ili9320_PutChar_cf(u16 x,u16 y,u8 c,u16 charColor,u16 bkColor)
{
    ...
    ili9320_SetWindows(x - widecharflag,y,x + 7 ,y + 15);
    ili9320_SetCursor(x - widecharflag,y);
    ...
    {           
        u16 *chdata;
        x = (lastch - 161) * 94 + c - 161;
        chdata = (u16 *)&Font_ch16x16[x * 32];
        for (i=0;i<16;i++)
        {
            for (j=0;j<16;j++)
            {
...
//	GPIOC->CRL = (GPIOC->CRL & 0x00000000) | 0x33333333;
    GPIOC->CRH = (GPIOC->CRH & 0x00000000) | 0x44444444;
	GPIOC->CRL = (GPIOC->CRL & 0x00000000) | 0x44444444;
    tmp = 10;
    while(tmp--);
    tmp = GPIOC->IDR;
    GPIOC->CRH = (GPIOC->CRH & 0x00000000) | 0x33333333;
    GPIOC->CRL = (GPIOC->CRL & 0x00000000) | 0x33333333;
	return tmp;
}
