...
unsigned int print_webpage(unsigned char *buf,unsigned char on_off)
    ...
    plen=fill_tcp_data_p(buf,plen,PSTR("</center><hr><br>STM32F103VBT6 基于UCOS-II V2.85 WEB 网页试验----阳志勇\n"));
...
//	Del_1ms(100);
	/*initialize enc28j60*/
	enc28j60Init(mymac);//ENC28J60初始化mymac[6] = {0x54,0x55,0x58,0x10,0x00,0x24};
	init_ip_arp_udp_tcp(mymac,myip,mywwwport);//init_ip_arp_udp_tcp(mymac,myip,mywwwport)
...
//		OSTimeDlyHMSM(0, 0, 0, 50);
        ...
        // iarp s broadcast if unknown but a host may also
        // verify the mac address by sending it to 
        // a unicast address.
        if(eth_type_is_arp_and_my_ip(buf,plen))
        	{
			make_arp_answer_from_request(buf);//ping 192.168.1.25 -t
			//USART_DMASendText(USART1,"make_arp_answer_from_request\n");
            continue;
        	}

        // check if ip packets are for us:
        if(eth_type_is_ip_and_my_ip(buf,plen)==0)//没有收到数据
        	{
            continue;
        	}
