...
void stm32_RtcSetup(void)
{
   ...
   if ((__RTC_CLKSRC_VAL & RCC_BDCR_RTCSEL) == 0x00000100)
   {
...
{
   if (__USART_USED & 0x01)
   {
      // USART1 used

      if ((__USART1_REMAP & 0x04) == 0x00)
      {
         // USART1 no remap
         RCC->APB2ENR |= RCC_APB2ENR_IOPAEN;                   // enable clock for GPIOA
   ...
   if (__USART_USED & 0x02)
   {
      // USART2 used

      if ((__USART2_REMAP & 0x08) == 0x00)
      {
         // USART2 no remap
         RCC->APB2ENR |= RCC_APB2ENR_IOPAEN;                  // enable clock for GPIOA
   ...
   if (__USART_USED & 0x04)
   {
      // USART3 used

      AFIO->MAPR &= ~(3 << 4);                              // clear USART3 remap
      if ((__USART3_REMAP & 0x30) == 0x00)
      {
         // USART3 no remap
         RCC->APB2ENR |= RCC_APB2ENR_IOPBEN;                  // enable clock for GPIOB
      ...
      else if ((__USART3_REMAP & 0x30) == 0x10)
      {
