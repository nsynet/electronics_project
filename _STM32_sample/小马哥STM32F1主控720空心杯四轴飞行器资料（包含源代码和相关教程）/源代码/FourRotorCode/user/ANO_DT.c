...
/************************************************************************
*数据拆分宏定义，在发送大于1字节的数据类型时，比如int16、float等，需要把
数据拆分成单独字节进行发送
*************************************************************************/
...
///***********************************************************************************
//Data_Exchange函数处理各种数据发送请求，比如想实现每5ms发送一次传感器数据至上位机，即
//在此函数内实现此函数应由用户每1ms调用一次
//************************************************************************************/
...
移植时，用户应根据自身应用的情况，和使用的通信方式，实现此函数。
由串口发送多个字节长度的数据
**************************************************************/
void ANO_DT_Send_Data(u8 *dataToSend , u8 length)
{
	...
	while(length>0){
		USART_SendData(USART,(u8)*dataToSend);
		dataToSend++;
		length--;
