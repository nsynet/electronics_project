...
/*
	AD9833模块可以直接插到STM32-V5开发板CN19排母(2*4P 2.54mm)接口上 （SPI）

	AD9833 模块    STM32-V5开发板
	  GND   ------  GND
	  VCC   ------  3.3V
      
	  FSYNC ------  PF7/NRF24L01_CSN
	  SCLK  ------  PB3/SPI3_SCK
	  SDATA ------  PB5/SPI3_MOSI 
...
/* 定义口线置0和置1的宏 */
#define AD9833_CS_0()	PORT_CS->BSRRH = PIN_CS
#define AD9833_CS_1()	PORT_CS->BSRRL = PIN_CS
...
void AD9833_SetCS(uint8_t _level)
{
	if (_level == 0)
	{
		...
		#ifdef HARD_SPI		/* 硬件SPI  -- AD9833 支持最快40MHz 的SCK速率. F407 实测 8分频时，SCK频率 = 10.5M */
