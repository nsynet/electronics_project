...
*********************************************************************************************************
*/
uint16_t g_usFirstTimeIRQFlag;  /* 第一次进入定时器8的中断标志 */
uint16_t g_usTrigCount = 0;     /* 记录普通触发进入看门狗中断时，DMA传输的位置 */
uint16_t g_usTrigTempFlag = 0;  /* 临时变量，用于ADC中断 */
...
*/
/* 
	每次捕获采集触发值前后的1024个ADC数据(单通道)，三通道就是完后1024*3个ADC数据。
*/
...
void TIM8_MeasureTrigConfig(void)
{
	...
	/* 定时器配置 */
	TimeBaseId = 1;   /* 开机后按照ADC单通道2Msps进行配置 */
	TIM_DeInit(TIM8);  
