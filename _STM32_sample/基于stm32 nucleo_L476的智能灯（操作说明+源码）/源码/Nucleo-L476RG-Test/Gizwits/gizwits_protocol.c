...
static int8_t gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLD2_OnOff)
    {
        info->event[info->num] = EVENT_LD2_ONOFF;
        info->num++;
        dataPoints->valueLD2_OnOff = gizDecompressionValue(LD2_ONOFF_BYTEOFFSET,LD2_ONOFF_BITOFFSET,LD2_ONOFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLD2_OnOff != cur->valueLD2_OnOff)
    {
        GIZWITS_LOG("valueLD2_OnOff Changed\n");
        ret = 1;
    }
    ...
    if(last->valueFault_Tem != cur->valueFault_Tem)
    {
        GIZWITS_LOG("valueFault_Tem Changed\n");
        ret = 1;
    }

    if(last->valueChipTemp != cur->valueChipTemp)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueChipTemp Changed\n");
...
static int8_t gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(LD2_ONOFF_BYTEOFFSET,LD2_ONOFF_BITOFFSET,LD2_ONOFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLD2_OnOff);
    ...
    devStatusPtr->valueFault_Tem = dataPoints->valueFault_Tem;

    devStatusPtr->valueChipTemp = gizY2X(CHIPTEMP_RATIO,  CHIPTEMP_ADDITION, dataPoints->valueChipTemp); 
